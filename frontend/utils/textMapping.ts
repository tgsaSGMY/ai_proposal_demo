...
interface SourceTypeMapType {
  [key: string]: MapEntry;
}

const MODE_MAP: ModeMapType = {
...
const SOURCE_TYPE_MAP: SourceTypeMapType = {
...
// 統一輸出函數
export function modeMap(status: string): string {
  return MODE_MAP[status]?.label || "未知";
}

export function getModeTypeClass(type: string): string {
